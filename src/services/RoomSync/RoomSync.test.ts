import reducer, { RoomState, RoomInfo, UserState, UserAction, enterRoom, exitRoom, addMember, updateMember, moveMember, removeMember, addAction, updateAction, moveAction, removeAction } from "./RoomSync";
...
describe("Test Cases for Reducers of RoomSync Service", () => {
  ...
  it("enterRoom", () => {
    state = {...initialState};
    expect(reducer(state, enterRoom({
      id: roomId,
      data: roomInfo,
    }))).toEqual({
      ...enteredState,
  ...
  it("updateMember", () => {
    ...
    expect(reducer(state, updateMember({
      id: "userid1",
      data: {
  ...
  it("moveMember", () => {
    ...
    expect(reducer(state, moveMember({
      id: "userid2",
      data: {...user2},
      previousChildName: null,
    }))).toEqual({
  ...
  it("removeMember", () => {
    ...
    expect(reducer(state, removeMember(
      "userid1"
    ))).toEqual({
      ...enteredState,
      members: {
  ...
  it("updateAction", () => {
    ...
    expect(reducer(state, updateAction({
      id: "actionid1",
      data: {
  ...
  it("moveAction", () => {
    ...
    expect(reducer(state, moveAction({
      id: "actionid2",
      data: {...action2},
      previousChildName: null,
    }))).toEqual({
  ...
  it("removeAction", () => {
    ...
    expect(reducer(state, removeAction(
      "actionid1"
    ))).toEqual({
      ...enteredState,
      actions: {
...
describe("Test Cases for Firebase Database Sync in RoomSync Service", () => {
  beforeEach(() => {
  });
  afterEach(() => {
  })
